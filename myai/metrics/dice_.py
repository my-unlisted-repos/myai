...
def sensitivity(input:torch.Tensor, target:torch.Tensor, ):
    """
    Sørensen–Dice coefficient often used for segmentation. Defined as two intersections over sum. Equivalent to F1 score.
    ...
    return ((intersection) / sum).nanmean(0) # mean along batch dim but not channel dim


def specificity(input:torch.Tensor, target:torch.Tensor, ):
    """
    Sørensen–Dice coefficient often used for segmentation. Defined as two intersections over sum. Equivalent to F1 score.
