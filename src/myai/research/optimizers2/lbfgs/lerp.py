...
class LBFGSLerp(Optimizer):
    """clip is applied like this (-lr*clip, lr*clip) cry about it."""
    def __init__(self, params, lr, z_beta=0.9, clip: float | None = None, g_beta=0, q_beta=0, history_size=10):
        defaults = dict(lr = lr, history_size = history_size,g_beta=g_beta, q_beta=q_beta, z_beta=z_beta)
        ...
        self.global_state['g'] = None
        self.global_state['q'] = None
        self.global_state['z'] = None
        self.global_state['dir'] = None
    ...
    def step(self, closure=None):
        ...
        grad = params.grad
        prev_params, prev_grad = self.state_vals('prev_params', 'prev_grad', init=['params', 'grad'])
        ...
        if self.global_state['step'] == 0:
            ...
            alpha_list = []
            q = grad.clone()
            if self.global_state['g'] is None: self.global_state['g'] = grad.clone()
            else: self.global_state['g'].lerp_(grad, 1-self.param_groups[0]['g_beta'])
            q = self.global_state['g'].clone()
            ...
            if self.global_state['q'] is None: self.global_state['q'] = q
            else: self.global_state['q'].lerp_(q, 1-self.param_groups[0]['q_beta'])
            q = self.global_state['q']

            z = q * (sy_k / (y_k.dot(y_k)))
            if self.global_state['z'] is None: self.global_state['z'] = z
            else: self.global_state['z'].lerp_(z, 1-self.param_groups[0]['z_beta'])
            z = self.global_state['z'].clone()
        ...
        if self.clip is not None: params.sub_(dir.clip(-lr*self.clip,lr*self.clip))
        else: params.sub_(dir.mul_(lr))
