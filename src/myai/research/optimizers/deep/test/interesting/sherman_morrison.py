...
class ShermanMorrison(Optimizer):
    ...
    def __init__(self, params, lr=1e-1, beta1=0.9, alpha=0.1, weight_decay=0, growth_bound = 1.2):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if self.growth_bound is not None:
                    norm = torch.linalg.vector_norm(p_grad)
                    if 'prev_norm' not in state: state['prev_norm'] = norm
                    else:
                        growth = norm / state['prev_norm']
                        if growth > self.growth_bound:
                            lr = lr / (growth / self.growth_bound)
                            state['prev_norm'] = state['prev_norm'] * self.growth_bound
                        else: state['prev_norm'] = norm
